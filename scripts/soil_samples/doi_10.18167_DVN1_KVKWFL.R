...
"
	...
	meta <- carobiner::get_metadata(uri, path, group, major=1, minor=1,
		...
		publication = "doi.org/10.1016/j.catena.2025.108843",
		project = "DSCATT",
		design = NA,
		data_type = "on-farm experiment",
		treatment_vars = "land_use;soil_type",
		response_vars = "soil_C_stock;soil_SOC;soil_N_total;depth;soil_type;soil_bd;soil_texture;soil_sand;soil_clay;soil_silt",
	...
	d <- data.frame(
	  ...
	  soil_texture = as.character(r$Texture_Class),
	  soil_clay = as.numeric(r$`Clay %`),
	  soil_silt = as.numeric(r$`Silt %`),
	  soil_sand = as.numeric(r$`Sand %`),
	  land_use = r$Landuse,
	  depth = as.character(as.numeric(r$Depth_1)),
	  #depth_top = as.character("0-20"),
	  #depth_bottom = as.character("20-40"),
...
location <-  data.frame(
  adm4 = c("Makombe", "Chitemerere", "Manjonjo"),
  latitude = c(-17.818, -17.822, -17.801),
  longitude = c(31.591, 31.619, 31.599)
 )
 
d <- merge(d, location, by = "adm4", all.x = TRUE)
